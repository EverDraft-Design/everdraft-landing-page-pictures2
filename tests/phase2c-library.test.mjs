...
assert.match(libraryHtml, /Create Account/);
...
assert.match(libraryJs, /library-card-actions/);
assert.match(libraryJs, /class="library-card-actions"[\s\S]*Read Story[\s\S]*library-spark-control/);
assert.match(libraryJs, /library-spark-control[\s\S]*library-follow-controls/);
...
assert.match(styles, /\.library-card-actions\s*\{[^}]*display:\s*flex/s);
assert.match(styles, /\.library-card-actions\s*\{[^}]*align-items:\s*center/s);
